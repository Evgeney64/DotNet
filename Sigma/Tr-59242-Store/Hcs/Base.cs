...
namespace Hcs
{
    ...
    public interface IDataStore : IDisposable
    {
        ...
        Task<IEnumerable<Guid>> CreateTransactionsAsync(TransactionInfo transaction);
        ...
        //Task<IEnumerable<Guid>> SetTransactionObjectsAsync(TransactionInfo transaction);
        ...
        Task<TransactionInfo2> AcquireTransactionAsync(TransactionInfo transactionInfo);
        Task ReleaseTransactionAsync(Guid transactionGuid, TransactionStatus transactionStatus);
        Task SetTransactionStatusAsync(Guid transactionGuid, TransactionStatus transactionStatus);
    }

    public interface IDataSource : IDisposable
    {
        Task<IEnumerable<T>> TakeDataAsync<T>(Guid transactionGuid, TransactionInfo transactionInfo)
            where T : class;
        Task PassDataAsync<T>(IEnumerable<T> data, Guid transactionGuid, TransactionInfo transactionInfo)
            where T : class;
        // note: :(
        Task<IEnumerable<ObjectInfo>> ListAsync(Guid listGuid, ListInfo listInfo);

        Task<ListInfo> ListInfoAsync();
    }
    ...
    {
        Task PostAttachmentAsync(IAttachment attachment);
    }

    public interface ICryptoService
    {
        Task<string> GetSignedDataAsync(string certificateThumbprint, string data);
        Task<byte[]> GetHashAsync(string certificateThumbprint, byte[] data);
    }
    ...
    // todo: разделить на две: с TransactionGuid и без
    public class TransactionInfo : ListInfo
    {
        ...
        public TransactionStatus InitialStatus { get; set; }
    ...
    public class TransactionInfo2 : TransactionInfo
    {
        public Guid TransactionGuid { get; set; }
    }

    public class TransactionInfo<T> : ListInfo
    {
        public IEnumerable<T> Request { get; set; }
    }
    ...
    public class TransactionResultInfo
    {
        ...
        public static TransactionResultInfo None(bool inProgress)
        {
            return new TransactionResultInfo
            {
                Result = TransactionResult.None,
                Status = inProgress ? TransactionStatus.InProgress : TransactionStatus.Ready,
            };
        }
        public static TransactionResultInfo TransactionError(ErrorInfo error, bool isCompleted)
        {
            return new TransactionResultInfo
            {
                ...
                Error = error,
                Status = isCompleted ? TransactionStatus.Completed : TransactionStatus.Ready,
            };
        }
