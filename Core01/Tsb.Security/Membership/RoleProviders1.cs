...
{
    public interface IFixedRoleProvider
    {
        string[] GetFixedRoles();
        bool IsFixedRole(string roleName);
    }

    public interface INestedRoleProvider : IFixedRoleProvider
    {
    ...
    public interface INestedDenyRoleProvider : IFixedRoleProvider
    {
        Tuple<string, bool>[] GetRolesForUser(string username);
        Tuple<string, bool>[] GetOwnRolesForUser(string username);
        Tuple<string, bool>[] GetRolesForRole(string roleName);
        Tuple<string, bool>[] GetOwnRolesForRole(string roleName);
        Tuple<string, bool>[] GetUsersInRole(string roleName);
        Tuple<string, bool>[] GetOwnUsersInRole(string roleName);
        Tuple<string, bool>[] GetRolesInRole(string roleName);
        Tuple<string, bool>[] GetOwnRolesInRole(string roleName);
        void AddRolesToRoles(Tuple<string, bool>[] addedRoleNames, string[] roleNames);
        void RemoveRolesFromRoles(string[] removedRoleNames, string[] roleNames);
    }
    ...
    public abstract class SuperRoleProvider : RoleProvider, INestedRoleProvider, IGroupRoleProvider
    {
        ...
        private void CreateRoleInternal(string roleName, bool isFixed, bool isDeny)
        {
            using (SecurityContext store = this.CreateContext())
            {
        ...
        public override void CreateRole(string roleName)
        {
            ...
            this.CreateRoleInternal(roleName, false, false);
        }
        ...
        public override string[] GetAllRoles()
        {
            using (SecurityContext store = this.CreateContext())
            {
                return store.GetRoles(false, false).Select(rl => rl.role_name).ToArray();
            }
        }
        ...
        public virtual Tuple<int, string>[] GetAllRoleIds()
        {
            using (SecurityContext store = this.CreateContext())
            {
                return store.GetRoles(false, false).AsEnumerable().Select(rl => new Tuple<int, string>(rl.role_id, rl.role_name)).ToArray();
            }
        }
        #endregion
        ...
        public virtual string[] GetFixedRoles()
        {
            using (SecurityContext store = this.CreateContext())
            {
                return store.GetRoles(true, false).Select(rl => rl.role_name).ToArray();
            }
        }
        ...
        #region INestedDenyRoleProvider members
        public string[] GetDenyRoles()
        {
            using (SecurityContext store = this.CreateContext())
            {
                return store.GetRoles(false, true).Select(rl => rl.role_name).ToArray();
            }
        }
        public void CreateRole(string roleName, bool isDeny)
        {
            if (!CheckRoleName(roleName, false))
            {
                throw new DuplicateRoleNameException();
            ...
            this.CreateRoleInternal(roleName, false, isDeny);
        }
        public bool IsDenyRole(string roleName)
        {
            using (SecurityContext store = this.CreateContext())
            {
                scr_role role = store.GetRoleByRoleName(roleName);
